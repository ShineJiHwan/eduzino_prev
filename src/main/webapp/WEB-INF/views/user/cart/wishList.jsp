...
let wishApp;
	...
	const wish_table={
		template:`
			<div  class="row">
			<input type="hidden"  :value="item.wish_idx">
				...
				<div class="eduzino-box-line subejct-item-checkbox">
				  <input type="checkbox" name="wish_idx" :value="item.wish_idx" />
				</div>
	...
	function getIndex(key){
		let wishList = wishApp.wishList;
		for(let i=0; i<wishList.length; i++){
			if(wishList[i].key == key) return i;
		}
		return -1;
	}
...
/*-------------------------------*/
	//let data=$("input[name=subject_idx]").is(':checked')
	...
	function delAsyncWish(){
		...
		//for(let i=0;i<$("input[name=wish_idx]:checked").length;i++){
			let json={};
			json['wish_idx']=parseInt($($("input[name=wish_idx]:checked")[0]).val());
			//json['wish_idx']=25;
			checkArray.push(json);
		//}
		
		//obj["wishList"]=checkArray;			
